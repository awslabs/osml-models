...
This CDK project deploys the infrastructure for running **OSML Models** on AWS, including SageMaker endpoints for the SAM3 (Segment Anything Model 3) model.
...
- **SAM3**: Uses `docker/Dockerfile.sam3` with PyTorch 2.7+ and CUDA 12.6+
...
| `modelEndpointConfig.CONTAINER_DOCKERFILE`   | string  | `"docker/Dockerfile.sam3"`     | Path to Dockerfile relative to build context                                           |
| `modelEndpointConfig.CONTAINER_BUILD_TARGET` | string  | `undefined`                    | Docker build target stage (for multi-stage builds)                                     |
| `modelEndpointConfig.INSTANCE_TYPE`          | string  | `"ml.g4dn.xlarge"`             | SageMaker instance type (SAM3 requires CUDA 12.6+ compatible instances)                |
| `modelEndpointConfig.INITIAL_INSTANCE_COUNT` | number  | `1`                            | Number of instances to deploy initially                                                |
| `modelEndpointConfig.MODEL_NAME`             | string  | `"sam3"`                       | Model name to load (sets `MODEL_SELECTION` environment variable)                       |
...
- Dockerfile location is specified by `CONTAINER_DOCKERFILE` (default: `docker/Dockerfile.sam3`)
...
- Try building manually: `docker build -f docker/Dockerfile.sam3 .`
...
#### ECR Push Broken Pipe / Proxy Timeout

**Error**:

```text
failed to copy: failed to do request: Put "https://...dkr.ecr...amazonaws.com/...": write tcp ...->...:3128: write: broken pipe
```

**Cause**: Docker Desktop is configured to use a proxy (commonly `http.docker.internal:3128`), and the proxy connection is timing out or being interrupted during large image pushes to ECR.

**Solution**: Configure Docker Desktop to bypass the proxy for ECR endpoints:

1. **Open Docker Desktop** → Settings → Resources → Proxies
2. **Add ECR domains to bypass list**:
   - `*.dkr.ecr.*.amazonaws.com`
   - `*.ecr.*.amazonaws.com`
   - Or your specific ECR endpoint: `012345678901.dkr.ecr.us-west-2.amazonaws.com`
3. **Apply & Restart** Docker Desktop
4. **Retry the deployment**

**Alternative**: If you cannot modify proxy settings, temporarily disable the proxy in Docker Desktop settings, restart Docker, and retry the push.
...
5. **Consult AWS Documentation**: [AWS Documentation](https://docs.aws.amazon.com/)
...
This CDK project provides infrastructure-as-code for deploying the OSML Models SAM3 model as a SageMaker endpoint. It includes security validations via cdk-nag and supports deployment across multiple environments through configuration files.
