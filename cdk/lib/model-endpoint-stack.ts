 ...
 * - SageMaker IAM execution role with necessary permissions
...
export class ModelEndpointStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: ModelEndpointStackProps) {
    ...
    const { deployment, vpc, selectedSubnets, securityGroup } = props;

    // Store VPC reference
    this.vpc = vpc;

    // Get subnet IDs from selected subnets
    const subnetIds = selectedSubnets.subnets?.map((s) => s.subnetId) ?? [];
    ...
    this.endpointName =
      this.resources.aircraftEndpoint.endpoint.endpointName || "";
