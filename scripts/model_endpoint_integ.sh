...
TEMP_PAYLOAD=$(mktemp)
echo "{\"image_uri\": \"s3://osml-models-test-imagery-$ACCOUNT_ID/2_planes.tiff\"}" > "$TEMP_PAYLOAD"

echo "Invoking the Lambda function 'OSML-Models-integration-test' with payload:"
echo "Payload: {\"image_uri\": \"s3://osml-models-test-imagery-$ACCOUNT_ID/2_planes.tiff\"}"
echo "Region: $AWS_REGION"
echo ""

# Check if Lambda function exists before invoking
if ! aws lambda get-function --function-name "OSML-Models-integration-test" --region "$AWS_REGION" &>/dev/null; then
    echo "ERROR: Lambda function 'OSML-Models-integration-test' not found in region $AWS_REGION"
    echo "Please ensure the infrastructure is deployed with the IntegrationTestStack"
    rm -f "$TEMP_PAYLOAD"
    exit 1
fi
...
if [ -n "$TIMEOUT_CMD" ]; then
    ...
    if ! log_result=$(${TIMEOUT_CMD} aws lambda invoke --region "$AWS_REGION" \
                                        --function-name "OSML-Models-integration-test" \
        ...
        echo "  aws logs tail /aws/lambda/OSML-Models-integration-test --follow --region $AWS_REGION"
        rm -f "$TEMP_PAYLOAD"
        exit 1
    fi
else
